...
public class CalculationTraceTests
{
    ...
    private readonly EngineFuelCurves _curves;
    ...
    public async Task Example1_BulkCarrier_TransitOnly_FullTrace()
    {
        ...
        var l1Result = _l1.FindOptimalCombination(input, _curves, OperationalMode.Transit);
        ...
        var l2Result = _l2.OptimizeLoadSetpoints(l1Result, input, _curves);
        ...
        var l3Result = _l3.CalculateDrcSavings(l2Result, input, _curves, input.TransitHours);
    ...
    public async Task Example2_BulkCarrier_TransitPlusDP_FullTrace()
    {
        ...
        // Transit pipeline
        _output.WriteLine($"\n── TRANSIT MODE ──");
        var transitL1 = _l1.FindOptimalCombination(input, _curves, OperationalMode.Transit);
        PrintL1Summary(transitL1, input.TransitHours);

        var transitL2 = _l2.OptimizeLoadSetpoints(transitL1, input, _curves);
        PrintL2Summary(transitL2, input.TransitHours);

        var transitL3 = _l3.CalculateDrcSavings(transitL2, input, _curves, input.TransitHours);
        ...
        var dpL1 = _l1.FindOptimalCombination(input, _curves, OperationalMode.DP);
        PrintL1Summary(dpL1, input.DPHours ?? 0);

        var dpL2 = _l2.OptimizeLoadSetpoints(dpL1, input, _curves);
        PrintL2Summary(dpL2, input.DPHours ?? 0);

        var dpL3 = _l3.CalculateDrcSavings(dpL2, input, _curves, input.DPHours ?? 0);
    ...
    public async Task Example4_Container_WithDP_FullTrace()
    {
        ...
        var tL1 = _l1.FindOptimalCombination(input, _curves, OperationalMode.Transit);
        PrintL1Summary(tL1, 4000);
        var tL2 = _l2.OptimizeLoadSetpoints(tL1, input, _curves);
        PrintL2Summary(tL2, 4000);
        var tL3 = _l3.CalculateDrcSavings(tL2, input, _curves, input.TransitHours);
        _output.WriteLine($"  L3 DRC: {tL3.DrcSavingsTonPerYear:F4} ton/yr");

        // DP
        _output.WriteLine($"\n── DP (propulsion=2000 kW, hotel=4000 kW, 1000h) ──");
        var dL1 = _l1.FindOptimalCombination(input, _curves, OperationalMode.DP);
        PrintL1Summary(dL1, 1000);
        var dL2 = _l2.OptimizeLoadSetpoints(dL1, input, _curves);
        PrintL2Summary(dL2, 1000);
        var dL3 = _l3.CalculateDrcSavings(dL2, input, _curves, input.DPHours ?? 0);
    ...
    public async Task Example5_LNG_L2Sweep_FullTrace()
    {
        ...
        // ── LEVEL 1 ──
        _output.WriteLine($"\n══ LEVEL 1 ══");
        var l1 = _l1.FindOptimalCombination(input, _curves, OperationalMode.Transit);
        PrintDetailedL1(l1);
        ...
        var l2 = _l2.OptimizeLoadSetpoints(l1, input, _curves);
        PrintDetailedL2(l2, opt);
        _output.WriteLine($"  L2 Savings/yr: {l2.Level2SavingsTonPerHour * input.TransitHours:F2} ton/yr");

        // ── LEVEL 3 ──
        _output.WriteLine($"\n══ LEVEL 3 (DRC) ══");
        var l3 = _l3.CalculateDrcSavings(l2, input, _curves, input.TransitHours);
        PrintDetailedL3(l3);
