...
public class Level2OptimizationServiceTests
{
    ...
    public Level2OptimizationServiceTests()
    {
        ...
        _level1Service = new Level1OptimizationService(Microsoft.Extensions.Options.Options.Create(new BatterySettings()));
        _service = new Level2OptimizationService();
    ...
    /// </summary>
    private static (Level1OptimizationService L1, Level2OptimizationService L2, EngineFuelCurves Curves)
        CreateUShapedServices()
    {
        ...
        var curves = sfocService.GetCurvesAsync(CalculatorInputBuilder.Default().Build())
            .GetAwaiter().GetResult();

        return (
            new Level1OptimizationService(Microsoft.Extensions.Options.Options.Create(new BatterySettings())),
    ...
    public async Task TwoGenerators_OptimalSetpointsMinimizeTotalSfoc()
    {
        ...
        var level1 = _level1Service.FindOptimalCombination(input, _curves, OperationalMode.Transit);

        level1.OptimalCombination.ActiveAeCount.Should().Be(2, "2 AEs must be active");

        var result = _service.OptimizeLoadSetpoints(level1, input, _curves);
    ...
    public async Task UShapedSfoc_TwoGenerators_BothActive_CoverDemand()
    {
        ...
        var (l1, l2, uCurves) = CreateUShapedServices();
        ...
        var level1 = l1.FindOptimalCombination(input, uCurves, OperationalMode.Transit);
        ...
        var result = l2.OptimizeLoadSetpoints(level1, input, uCurves);
    ...
    public async Task GeneratorOFF_SmallAeCoversAll_LargeAeOff()
    {
        ...
        var level1 = factory.Level1Service.FindOptimalCombination(input, factory.Curves, OperationalMode.Transit);

        // If L1 picked 2 AEs, L2 should find one OFF is better
        if (level1.OptimalCombination.ActiveAeCount >= 2)
        {
            var result = factory.Level2Service.OptimizeLoadSetpoints(level1, input, factory.Curves);
