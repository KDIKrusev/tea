...
public class Level1OptimizationServiceTests
{
    ...
    public Level1OptimizationServiceTests()
    {
        ...
        _service = new Level1OptimizationService(Microsoft.Extensions.Options.Options.Create(new BatterySettings()));
    ...
    public async Task Transit_BulkCarrierSetup_Returns2ValidCombinations_WhenSgCoversHotel()
    {
        ...
        // Act
        var result = _service.FindOptimalCombination(input, _curves, OperationalMode.Transit);
    ...
    public async Task Transit_AllMeZeroCombinations_AreExcluded()
    {
        ...
        // Pin the candidate set: without it, "ME=0 was excluded" would keep passing even if the
        // set collapsed to a single survivor and the filter stopped discriminating.
        result.AllValidCombinations.Should().HaveCount(2);
    ...
    public async Task DP_ValidCombinationsExcludeSgOnWithoutMe()
    {
        ...
        var result = _service.FindOptimalCombination(input, _curves, OperationalMode.DP);
    ...
    public async Task DP_MeZeroWithInsufficientAe_FilteredByPowerCheck()
    {
        ...
        // NOTE: this plant leaves exactly ONE survivor, so OnlyContain below checks a single
        // combination — the count assertion is what actually guards the filter here.
