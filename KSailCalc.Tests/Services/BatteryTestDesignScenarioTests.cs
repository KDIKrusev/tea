...
public class BatteryTestDesignScenarioTests
{
    ...
    public async Task C4_DischargeGate_BandExactlyEqualToHeadroom_IsKept()
    {
        ...
        var result = await factory.Level1Service.FindOptimalCombinationAsync(
            input, OperationalMode.Transit, batteryAdjustment: adjustment);
    ...
    {
        var factory = TestServiceFactory.Create();
        var level1 = new Level1OptimizationService(
            factory.SfocService, Options.Create(new BatterySettings { PtiLossFactor = 0.10 }));
        var input = DeficitPlant().Build();
        input.MaxPtiPerEngineKw = 500;

        var result = await level1.FindOptimalCombinationAsync(input, OperationalMode.Transit);
    ...
    public async Task C8_PtiKw_ExposedInValidCombinationDto()
    {
        ...
        var withPti = Level1Details.FromResult(
            await factory.Level1Service.FindOptimalCombinationAsync(deficit, OperationalMode.Transit));
        ...
        var withoutPti = Level1Details.FromResult(
            await factory.Level1Service.FindOptimalCombinationAsync(noDeficit, OperationalMode.Transit));
    ...
    [Fact]
    public async Task G4_LegacyBatteryCapacityField_HasNoEffectWhateverItsValue()
    {
        var factory = TestServiceFactory.Create();

        var zero = CalculatorInputBuilder.Default().Build();
        zero.BatteryCapacity = 0;
        var large = CalculatorInputBuilder.Default().Build();
        large.BatteryCapacity = 999_999;

        var a = await factory.CalculatorService.CalculateAllVariantsAsync(zero);
        var b = await factory.CalculatorService.CalculateAllVariantsAsync(large);

        b.BaselineFOC.Should().Be(a.BaselineFOC);
        b.Premium.FuelSavings.Should().Be(a.Premium.FuelSavings);
        b.BatteryDetails.Should().BeNull();
    }
}
